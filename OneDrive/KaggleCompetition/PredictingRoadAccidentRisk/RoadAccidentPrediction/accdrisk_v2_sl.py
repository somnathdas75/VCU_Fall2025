...
# 1. Load train.csv and test.csv from Kaggle input path
...
# Function to load data from Kaggle path
@st.cache_data
def load_kaggle_data():
    """
    Load train and test data from Kaggle input path
    ...
    try:
        ...
        st.success("✅ Data loaded successfully from Kaggle path!")
        return train_data, test_data
        
    except FileNotFoundError:
        st.error("❌ Data files not found at the specified Kaggle path.")
        st.info("""
        If running locally, please:
        1. Download the data from Kaggle: https://www.kaggle.com/competitions/playground-series-s5e10
        2. Update the file paths in the code to match your local directory
        3. Or use the file upload option below
        """)
        return None, None
...
# Alternative function for file upload
@st.cache_data
def load_uploaded_data(train_file, test_file):
    """
    Load data from uploaded files
    
    Parameters:
    - train_file: uploaded train CSV file
    - test_file: uploaded test CSV file
...
def validate_data(train_data, test_data):
    ...
    train_missing = [col for col in required_train_cols + required_common_cols if col not in train_data.columns]
    test_missing = [col for col in required_common_cols + ['id'] if col not in test_data.columns]
    
    if train_missing:
        st.error(f"❌ Missing columns in training data: {train_missing}")
        return False
    
    if test_missing:
        st.error(f"❌ Missing columns in test data: {test_missing}")
        return False
...
data_source = st.sidebar.radio(
    "Select Data Source",
    ["Kaggle Path", "Upload Files"]
)

if data_source == "Kaggle Path":
    # Load data from Kaggle path
    if st.sidebar.button("Load Data from Kaggle Path"):
        with st.spinner("Loading data from Kaggle path..."):
            train_data, test_data = load_kaggle_data()
            
            if validate_data(train_data, test_data):
    ...
    if train_file is not None and test_file is not None:
        with st.spinner("Loading uploaded data..."):
            train_data, test_data = load_uploaded_data(train_file, test_file)
...
if st.session_state.train_data is not None and st.session_state.test_data is not None:
    ...
    st.info("👆 Please load data using the sidebar options to begin.")
    
    st.markdown("""
    ## 📋 Instructions
    
    ### Step 1: Load Data
    - **Option 1 (Kaggle Path)**: Use the pre-configured Kaggle path (requires Kaggle environment)
    - **Option 2 (Upload Files)**: Upload train.csv and test.csv files manually
    ...
    Ready to begin? Load your data using the sidebar options! 👈
    """)
